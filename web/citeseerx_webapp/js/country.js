//<!--
// IGC, based on a script from Jim Carlock
// 
...
/* 
 * Working on this one.
 * Fills in province from the arrays
 * 
 */
function xFillState() {
 ...
 // reset province
 document.accountForm["account.province"].options.length=0;
 
 // get selected country
 gLngSelectedCountry=document.accountForm["account.country"].selectedIndex;
 
 // get number of states for selected country
 gLngNumberStates=gArCountryInfo[[gLngSelectedCountry][2]];
 
 // update options in province
 for (i=0;i<gLngNumberStates;i++) {
  document.accountForm["account.province"].options[i]=new
    Option(gArStateInfo[[gLngSelectedCountry][i][0]]);
 }
 gLngSelectedState=
  document.accountForm["account.province"].options.selectedIndex;
 ...
 * Fills province from aStates
 * countryElID: Countries select element id
 * provinceElID: Provinces select element id
 */
function Fill_States(countryElID, provinceElID) {
 var i=0, iLen=0, iNumStates=0;

 var countryElt = document.getElementById(countryElID);
 var stateElt = document.getElementById(provinceElID);
 var priorValue = stateElt.value;
 var priorIndex = -1;

 // reset province
 stateElt.options.length=0;
 // get selected country
 gLngSelectedCountry=countryElt.selectedIndex;
 iNumStates = aStates[gLngSelectedCountry].split("|").length;
 
 // file the state combobox with the list of states
 for (i=0;i<iNumStates;i++) {
  var option = new Option(aStates[countryElt.selectedIndex].split("|")[i]);
  option.value = option.text; 
  stateElt.options[i] = option;
  if (option.value == priorValue) {
    priorIndex = i;
  }
 }

 if (priorIndex >= 0) {
   stateElt.selectedIndex=priorIndex;
 }
 return;
}
 ...
 * (3) Max state length
 */
function Fill_Country(countryElID) {
 var i=0;
 var sCountryName="";
 
 var countryElt = document.getElementById(countryElID);
 var priorValue = countryElt.value;
 var priorIndex = -1;

 // reset country.options
 countryElt.options.length=0;
 ...
 for (i=0;i<gLngNumberCountries;i++) {
  countryElt.options[i]=new Option(sCountryString.split("|")[i]);
  countryElt.options[i].value = countryElt.options[i].text;
  sCountryName=countryElt.options[i];

  if (sCountryName.value == priorValue) {
    priorIndex = i;
  }
  
  //document.accountForm.txtState.value=sCountryName;
 ...
 if (priorIndex >= 0) {
  countryElt.selectedIndex = priorIndex;
 }
 return;
}
