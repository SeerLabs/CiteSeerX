...
<ul>
  <li class="active"><a href="<c:url value="/stats/articles"/>" >Most Cited Articles</a></li>
...
<div id="content">
  <div id="statsHeader">
    <h2>Most Cited Computer Science Articles</h2>
  ...
  <div id="statsSubMenu">
    <c:if test="${ empty currentlink }"><a class="active" href="<c:url value="/stats/articles"/>">All Years</a> </c:if>
  ...
  <div id="statsList">
   <ol start="<c:out value="${ start+1 }"/>">
    <c:forEach var="doc" items="${ docs }">
     <c:if test="${ doc.inCollection }">
      <li>
        ...
        <a href="<c:url value="/viewdoc/summary?cid=${ doc.cluster }"/>"><c:out value="${ doc.title }"/></a>. <c:out value="${ doc.venue }"/>
	...
	<div class="cites"><c:out value="${ doc.ncites }"/></div>
     ...
     <c:if test="${ ! doc.inCollection }">
      <li>
        <div class="bibcite">
        <span class="authors"><c:out value="${ doc.authors }"/>.</span>
   ...
   <div id="statsNav">
    <c:if test="${ ! empty nextPageParams }">
     <a href="<c:url value="/stats/articles?${ nextPageParams }"/>">Next <c:out value="${ pageSize }"/> &#8594;</a>  
    </c:if>
   </div> 
